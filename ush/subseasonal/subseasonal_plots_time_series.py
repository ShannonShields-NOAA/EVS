...
class TimeSeries:
    ...
    def make_time_series(self):
        ...
        if self.plot_info_dict['stat'] == 'FBAR_OBAR':
            stat_df, stat_array = sub_util.calculate_stat(
                self.logger, all_model_df, self.plot_info_dict['line_type'],
                'FBAR'
            )
            obar_stat_df, obar_stat_array = sub_util.calculate_stat(
                self.logger, all_model_df, self.plot_info_dict['line_type'],
                'OBAR'
            )
        else:
        ...
        for model_idx in model_idx_list:
            ...
            if model_num_npts != 0:
                ...
                if self.plot_info_dict['stat'] == 'FBAR_OBAR':
                    model_num_avg = sub_util.calculate_average(
                        self.logger, avg_method,
                        self.plot_info_dict['line_type'],
                        'FBAR', calc_avg_df
                    )
                    obar_model_num_avg = sub_util.calculate_average(
                        self.logger, avg_method,
                        self.plot_info_dict['line_type'],
