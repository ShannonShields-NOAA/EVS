#!/usr/bin/env python3
'''
Name: global_det_nwp_index_plots_time_series.py
...
class TimeSeries:
    ...
    def make_time_series(self):
        ...
        for model_num in list(self.model_info_dict.keys()):
            nwp_stat = os.path.join(
                self.input_dir, 'condensed_stats_'
                +f"{self.model_info_dict[model_num]['name'].lower()}_"
                +f"{self.plot_info_dict['line_type'].lower()}_"
