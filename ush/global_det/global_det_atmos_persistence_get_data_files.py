#!/usr/bin/env python3
'''
Name: global_det_atmos_persistence_get_data_files.py
Contact(s): Shannon Shields (shannon.shields@noaa.gov)
Abstract: This gets the necessary data files for persistence verification.
Run By: scripts/stats/global_det/exevs_stats_global_det_atmos_grid2grid_persistence.sh
'''

import os
import datetime
import global_det_atmos_util as gda_util
import global_det_persistence_util as gdp_util
...
if VERIF_CASE_STEP == 'grid2grid_stats':
    # Create persistence forecasts and get truth files for
    ...
    for VERIF_CASE_STEP_type in VERIF_CASE_STEP_type_list:
        ...
        # Create persistence forecast files for each model
        ...
        for time in VERIF_CASE_STEP_type_time_info_dict:
            ...
            for model_idx in range(len(model_list)):
                ...
                if model in ['cfs', 'fnmoc', 'gfs', 'aigfs', 'aifs']:
                ...
                elif model in ['cmc', 'ecmwf', 'ukmet']:
                    gdp_util.patch_grib1_files(
                        time['valid_time'], time['init_time'],
                        time['forecast_hour'],
                        model_file_format, model_fcst_dest_file_format,
                        VERIF_CASE_STEP_model_dir
                    )
                    grib2_file_format = os.path.join(
                        VERIF_CASE_STEP_model_dir,
                        'patched_grib2_files',
                        model+'.'+'{init?fmt=%Y%m%d%H}.f000.grib2'
                    )
                    gdp_util.create_grib2_persistence_files(
                        time['valid_time'], time['init_time'],
                        time['forecast_hour'], grib2_file_format,
                        model_fcst_dest_file_format
                    )
                if VERIF_CASE_STEP_type == 'flux':
