#!/usr/bin/env python3
'''
Name: global_det_atmos_stats_grid2grid_persistence_create_job_scripts.py
...
gather_stats_jobs_dict = {'env': {},
                    'commands': [gda_util.metplus_command(
                                     'StatAnalysis_fcstGLOBAL_DET_persistence.conf'
                                 )]}
...
if JOB_GROUP in ['reformat_data', 'assemble_data', 'generate_stats']:
    ...
    while date_dt <= end_date_dt:
        ...
        for model_idx in range(len(model_list)):
            ...
            job.write('\n')
            # Do file checks
            stat_files_exist, copy_output_list = gdp_util.check_stat_files(
                job_env_dict
            )
            if stat_files_exist:
                write_job_cmds = True
