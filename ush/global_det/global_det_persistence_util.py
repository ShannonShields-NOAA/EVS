#!/usr/bin/env python3
'''
Name: global_det_persistence_util.py
Contact(s): Shannon Shields (shannon.shields@noaa.gov)
Abstract: This contains specific functions used for global_det persistence stats.
'''
...
def convert_grib1_grib2(grib1_file, grib2_file):
    ...
    run_shell_command(
        [cnvgrib, '-g12', '-nv', grib1_file, grib2_file, '>', '/dev/null', '2>&1']
    )
...
def prep_prod_ukmet_file(source_file, dest_file, init_dt,
    ...
    if prep_method == 'full':
        if check_file_exists_size(source_file):
            if not check_grib1_file_corrupt(source_file):
               # === Check if ukmet file contains all the required vairables 
...
def create_grib2_persistence_files(valid_time_dt, init_time_dt,
                                   forecast_hour, source_file_format,
                                   dest_file_format):
    """! Modifies F000 files to create persistence forecasts
    ...
    if not os.path.exists(dest_file):
        ...
        if write_file:
            if check_file_exists_size(source_file):
                # Update metadata from 'anl' to 'fcst'
                # and set the specific forecast hour
                hourcmd = [
                    "wgrib2", source_file,
                    "-set_ftime", f"{forecast_hour} hour fcst",
                    "-grib", dest_file
                ]
                print(f"Running: {' '.join(hourcmd)}")
                subprocess.run(hourcmd)
                print(f"Created GRIB2 Persistence: {dest_file}")
            else:
...
def patch_grib1_files(valid_time_dt, init_time_dt, forecast_hour,
                      source_file_format, dest_file_format,
                      DATA_dir):
    """! Uses wgrib to isolate a single grib1 record, convert to grib2,
         and then rename variable
         ...
         Args:
             ...
             source_file_format - source file format (string)
             dest_file_format   - destination file format (string)
             DATA_dir           - main data working directory
    ...
    if not os.path.exists(dest_file):
        ...
        if check_file_exists_size(source_file):
            # HGT
            hgt_tmp_file_format = os.path.join(DATA_dir, 'hgt500.{init?fmt=%Y%m%d%H}.f000')
            hgt_tmp_file = format_filler(hgt_tmp_file_format, valid_time_dt,
                                         init_time_dt, '000', {})
            # Use wgrib to isolate a single grib1 record
            run_shell_command(
                ['wgrib '+source_file+' | grep "HGT" | grep "500 mb" | wgrib -i '+source_file
                 +' -grib -o '+hgt_tmp_file]
            )
            hgt_test_file = hgt_tmp_file+'.test.grib2'
            # Convert to grib2 using cnvgrib
            convert_grib1_grib2(hgt_tmp_file, hgt_test_file)
            patched_grib2_file_dir = os.path.join(DATA_dir, 'patched_grib2_files')
            make_dir(patched_grib2_file_dir)
            hgt_grib2_file_format = os.path.join(
                patched_grib2_file_dir,
                'hgt500.{init?fmt=%Y%m%d%H}.f000.grib2'
            )
            hgt_grib2_file = format_filler(hgt_grib2_file_format, valid_time_dt,
                                           init_time_dt, '000', {})
            # Use set_var in wgrib2 to rename the individual "IMGD"
            run_shell_command(
                ['wgrib2 '+hgt_test_file+' -set_var HGT -grib '+hgt_grib2_file]
            )
            # PRMSL
            prmsl_tmp_file_format = os.path.join(DATA_dir, 'prmsl.{init?fmt=%Y%m%d%H}.f000')
            prmsl_tmp_file = format_filler(prmsl_tmp_file_format, valid_time_dt,
                                           init_time_dt, '000', {})
            run_shell_command(
                ['wgrib '+source_file+' | grep "PRMSL" | grep "MSL" | wgrib -i '+source_file
                 +' -grib -o '+prmsl_tmp_file]
            )
            prmsl_test_file = prmsl_tmp_file+'.test.grib2'
            convert_grib1_grib2(prmsl_tmp_file, prmsl_test_file)
            prmsl_grib2_file_format = os.path.join(
                patched_grib2_file_dir,
                'prmsl.{init?fmt=%Y%m%d%H}.f000.grib2'
            )
            prmsl_grib2_file = format_filler(prmsl_grib2_file_format, valid_time_dt,
                                             init_time_dt, '000', {})
            run_shell_command(
                ['wgrib2 '+prmsl_test_file+' -set_var PRMSL -grib '+prmsl_grib2_file]
            )
            # UGRD at P850
            u850_tmp_file_format = os.path.join(DATA_dir, 'u850.{init?fmt=%Y%m%d%H}.f000')
            u850_tmp_file = format_filler(u850_tmp_file_format, valid_time_dt,
                                          init_time_dt, '000', {})
            run_shell_command(
                ['wgrib '+source_file+' | grep "UGRD" | grep "850 mb" | wgrib -i '+source_file
                 +' -grib -o '+u850_tmp_file]
            )
            u850_test_file = u850_tmp_file+'.test.grib2'
            convert_grib1_grib2(u850_tmp_file, u850_test_file)
            u850_grib2_file_format = os.path.join(
                patched_grib2_file_dir,
                'u850.{init?fmt=%Y%m%d%H}.f000.grib2'
            )
            u850_grib2_file = format_filler(u850_grib2_file_format, valid_time_dt,
                                            init_time_dt, '000', {})
            run_shell_command(
                ['wgrib2 '+u850_test_file+' -set_var UGRD -grib '+u850_grib2_file]
            )
            # VGRD at P850
            v850_tmp_file_format = os.path.join(DATA_dir, 'v850.{init?fmt=%Y%m%d%H}.f000')
            v850_tmp_file = format_filler(v850_tmp_file_format, valid_time_dt,
                                          init_time_dt, '000', {})
            run_shell_command(
                ['wgrib '+source_file+' | grep "VGRD" | grep "850 mb" | wgrib -i '+source_file
                 +' -grib -o '+v850_tmp_file]
            )
            v850_test_file = v850_tmp_file+'.test.grib2'
            convert_grib1_grib2(v850_tmp_file, v850_test_file)
            v850_grib2_file_format = os.path.join(
                patched_grib2_file_dir,
                'v850.{init?fmt=%Y%m%d%H}.f000.grib2'
            )
            v850_grib2_file = format_filler(v850_grib2_file_format, valid_time_dt,
                                            init_time_dt, '000', {})
            run_shell_command(
                ['wgrib2 '+v850_test_file+' -set_var VGRD -grib '+v850_grib2_file]
            )
            # UGRD at P250
            u250_tmp_file_format = os.path.join(DATA_dir, 'u250.{init?fmt=%Y%m%d%H}.f000')
            u250_tmp_file = format_filler(u250_tmp_file_format, valid_time_dt,
                                          init_time_dt, '000', {})
            run_shell_command(
                ['wgrib '+source_file+' | grep "UGRD" | grep "250 mb" | wgrib -i '+source_file
                 +' -grib -o '+u250_tmp_file]
            )
            u250_test_file = u250_tmp_file+'.test.grib2'
            convert_grib1_grib2(u250_tmp_file, u250_test_file)
            u250_grib2_file_format = os.path.join(
                patched_grib2_file_dir,
                'u250.{init?fmt=%Y%m%d%H}.f000.grib2'
            )
            u250_grib2_file = format_filler(u250_grib2_file_format, valid_time_dt,
                                            init_time_dt, '000', {})
            run_shell_command(
                ['wgrib2 '+u250_test_file+' -set_var UGRD -grib '+u250_grib2_file]
            )
            # VGRD at P250
            v250_tmp_file_format = os.path.join(DATA_dir, 'v250.{init?fmt=%Y%m%d%H}.f000')
            v250_tmp_file = format_filler(v250_tmp_file_format, valid_time_dt,
                                          init_time_dt, '000', {})
            run_shell_command(
                ['wgrib '+source_file+' | grep "VGRD" | grep "250 mb" | wgrib -i '+source_file
                 +' -grib -o '+v250_tmp_file]
            )
            v250_test_file = v250_tmp_file+'.test.grib2'
            convert_grib1_grib2(v250_tmp_file, v250_test_file)
            v250_grib2_file_format = os.path.join(
                patched_grib2_file_dir,
                'v250.{init?fmt=%Y%m%d%H}.f000.grib2'
            )
            v250_grib2_file = format_filler(v250_grib2_file_format, valid_time_dt,
                                            init_time_dt, '000', {})
            run_shell_command(
                ['wgrib2 '+v250_test_file+' -set_var VGRD -grib '+v250_grib2_file]
            )
            # Final patched grib2 file containing all variables
            patched_grib2_file = hgt_grib2_file.replace('hgt500', model)
            run_shell_command([
                f'cat {hgt_grib2_file} {prmsl_grib2_file} {u850_grib2_file} '
                f'{v850_grib2_file} {u250_grib2_file} {v250_grib2_file} '
                f'> {patched_grib2_file}'
            ])
        else:
            print(f"NOTE: {source_file} does not exist")
...
def check_model_files(job_dict):
    """! Check what model files or don't exist
         ...
         Returns:
             ...
             copy_output_list  - list of files to copy
    """
        ...
        if job_dict['JOB_GROUP'] == 'reformat_data':
            ...
            if job_dict['VERIF_CASE'] == 'grid2grid':
                if job_dict['VERIF_TYPE'] in ['precip_accum24hr',
                                              'precip_accum3hr']:
                    input_file_format = os.path.join(
                        verif_case_dir, 'data', model, model+'.precip.'
                        +'{init?fmt=%Y%m%d%H}.f{lead?fmt=%3H}'
                    )
                    ...
                    input_file_format = os.path.join(
                        ...
                        job_dict['RUN']+'.{valid?fmt=%Y%m%d}',
                        job_dict['MODEL'], job_dict['VERIF_CASE'],
                        'anomaly_'+job_dict['VERIF_TYPE']+'_'
                        +job_dict['job_name'].replace('DailyAvg_', '')
                    ...
                    input_file_format = os.path.join(
                        ...
                        'grid_stat_'+job_dict['VERIF_TYPE']+'_'
                        +job_dict['job_name'].replace('DailyAvg_', '')
                        +'_{lead?fmt=%2H}0000L_{valid?fmt=%Y%m%d}_'
                    ...
                    input_file_format = os.path.join(
                        verif_case_dir, 'data', model, model
                        +'.{init?fmt=%Y%m%d%H}.f{lead?fmt=%3H}'
                    )
            ...
            if job_dict['VERIF_CASE'] == 'grid2grid':
                ...
                if job_dict['VERIF_TYPE'] == 'ptype' \
                    ...
                    input_file_format = os.path.join(
                        ...
                        job_dict['RUN']+'.{valid?fmt=%Y%m%d}',
                        job_dict['MODEL'], job_dict['VERIF_CASE'],
                        'merged_ptype_'+job_dict['VERIF_TYPE']+'_'
                        +job_dict['job_name']+'_init{init?fmt=%Y%m%d%H}_'
                        +'fhr{lead?fmt=%3H}.nc'
                    )
                    ...
                    input_file_format = os.path.join(
                        ...
                        'anomaly_'+job_dict['VERIF_TYPE']+'_'
                        +job_dict['job_name'].replace('DailyAvg_', '')
                        +'_init{init?fmt=%Y%m%d%H}_fhr{lead?fmt=%3H}.stat'
                    )
                else:
                    input_file_format = os.path.join(
                        verif_case_dir, 'data', model,
    ...
    # Check output files
    output_file_format_DATA = output_file_format_JOB.replace(
        job_dict['job_num_work_dir'],
        os.path.join(verif_case_dir, 'METplus_output')
    )
    output_file_format_COMOUT = output_file_format_JOB.replace(
        ...
        for fhr_fileN_key in list(fhr_check_output_dict[fhr_key].keys()):
            ...
            if os.path.exists(fhr_fileN_COMOUT):
                ...
                if fhr_check_output_dict[fhr_key]\
                        [fhr_fileN_key]['forecast_hour'] \
                        in fhr_list:
                    output_tuple = (fhr_fileN_JOB, fhr_fileN_COMOUT)
...
def check_stat_files(job_dict):
    ...
    file_JOB = os.path.join(
        ...
        +f"{job_dict['VERIF_CASE']}.persistence.v{job_dict['DATE']}.stat"
    )
        ...
        for plot_info in plot_info_list:
            ...
            if plot_dict['plot'] == 'time_series':
                ...
                if int(plot_dict['forecast_hour']) > 72 \
                    ...
                    if 'cmc_regional' in psm_model_list:
