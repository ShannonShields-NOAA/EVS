#!/usr/bin/env python3
'''
Name: global_det_atmos_nwp_index_util.py
...
def check_nwp_index_file_exists_size(file_name):
    """! Checks to see if file exists and has size greater than 600 bytes
         ...
         Returns:
             file_good - boolean
                       - True: file exists,file size > 600
                       - False: file doesn't exist
                                OR file is empty
    """
    if '/com/' in file_name or '/dcom/' in file_name:
        ...
        alert_word = 'NOTE'
    if os.path.exists(file_name):
        if os.path.getsize(file_name) > 600:
            file_good = True
        else:
            print(f"{alert_word}: {file_name} is empty")
            file_good = False
    else:
        print(f"{alert_word}: {file_name} does not exist")
        file_good = False
